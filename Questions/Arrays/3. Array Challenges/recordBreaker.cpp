...
int recordBreaker(int arr[], int size)
{
    ...
    {
        //condition if the first element is a record breaking
        // if (arr[i] == arr[0] && arr[i] > arr[1])
        // {
        //     recordBreakingDays = recordBreakingDays + 1;
        //     continue;
        // }

        if (arr[i] > max && (arr[i] > arr[i + 1] || arr[i] == arr[size - 1]))
        {
            recordBreakingDays = recordBreakingDays + 1;
        }
...
int main()
{
    ...
    int answer = recordBreaker(arr, 8);
    cout << answer;
}